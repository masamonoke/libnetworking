...
static void test_setup(void) {
	const char* port = "100";
	networking_server_select_new(&server, port, callback);
}

static void test_teardown(void) {
	networking_server_select_del(server);
}
...
MU_TEST(test_handle) {
	...
	if (p == 0) {
		...
		mu_check(networking_server_select_update(server, NULL) == NETWORKING_SERVER_OK);
	}
	wait(NULL);
}
